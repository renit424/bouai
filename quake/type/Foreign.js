    ...
    fetch(url).then(function (response) {
      return response.json();
        ...
        fetch(url2).then(function (response) {
          ...
          var info = document.getElementById("info"); info.innerHTML = "遠方地震情報<br>発生時刻 " + json[0].earthquake.time.split('/').join('月').split(' ').join('日').slice(5) + "<br>震源地 " + json[0].earthquake.hypocenter.name + "<br>深さ " + json[0].earthquake.hypocenter.depth + "km<br>規模(M) " + json[0].earthquake.hypocenter.magnitude + "<br>国内津波 "+ tsunami + "<br>海外津波 " + foreign;
          var myIcon = L.icon({
            iconUrl: 'icon.png',
            iconSize: [30, 30]
          }); L.marker([json[0].earthquake.hypocenter.latitude, json[0].earthquake.hypocenter.longitude], {
            icon: myIcon
          }).bindPopup("<span class=font_size>震源地：" + json[0].earthquake.hypocenter.name + "</span>").addTo(map); len = Object.keys(json[0]['points']).length;
          }
        });
    });
}
